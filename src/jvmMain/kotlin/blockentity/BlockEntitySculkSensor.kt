...
import org.chorus_oss.chorus.block.BlockSculkSensor
...
class BlockEntitySculkSensor(level: Level, nbt: CompoundTag) : BlockEntity(level, nbt),
    VibrationListener {
    ...
    override fun onVibrationArrive(event: VibrationEvent) {
        if (this.isBlockEntityValid && Server.instance.settings.levelSettings.enableRedstone) {
            ...
            val block = this.block as BlockSculkSensor
            block.setPhase(1)
